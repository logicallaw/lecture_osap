...
class AirPodsAdapter : public AirPodsInterface {
public:
	AirPodsAdapter(AirPods *airpod) {
		this->airpod = airpod;
		cout << "AirPods이 SamsungPhone과 호환됩니다." << endl;
	}
	void play() override { this->airpod->playing(); }
	void stop() override { this->airpod->stopping(); }
...
public:
	...
	void installAirPods() {
		airpodsadapter = new AirPodsAdapter(new AirPods);
